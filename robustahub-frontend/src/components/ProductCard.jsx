...
const ProductCard = ({ product, isBestSeller }) => {
  ...
  const handleAddToCart = () => {
    const user = localStorage.getItem('user'); // Cek apakah sedang login
    
    if (!user) {
      // Jika belum login, cegat dan pindahkan ke halaman login
      alert('Silakan masuk (login) ke akun Anda terlebih dahulu untuk mulai berbelanja.');
      navigate('/');
    } else {
      // TODO: Logika masukkan ke keranjang di backend ntar di sini
      alert(`${product.name} siap ditambahkan ke keranjang! (Fitur segera hadir)`);
    }
  };
    ...
    <div className="bg-white rounded-[16px] overflow-hidden border border-[#EFEFEF] transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_16px_40px_rgba(0,0,0,0.08)] hover:border-transparent flex flex-col relative group">
      ...
      <div className="p-6 flex flex-col flex-1">
        ...
        <div className="mt-auto border-t border-[#EFEFEF] pt-5 flex justify-between items-center">
          ...
          {/* PASANGKAN FUNGSI handleAddToCart DI TOMBOL INI */}
          <button 
            onClick={handleAddToCart}
            className="w-10 h-10 rounded-full bg-[#FDF9F5] text-[#A86431] border-none cursor-pointer flex items-center justify-center transition-all duration-300 group-hover:bg-[#A86431] group-hover:text-white" 
